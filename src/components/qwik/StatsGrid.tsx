  ...
  return (
    <div class="grid grid-cols-5 gap-3 mt-4 pt-4 border-t border-gray-100">
      {stats.map((stat) => (
        ...
        >
          <div
            class={`text-2xl font-bold text-${stat.color}-600 group-hover:scale-105 transition-transform flex items-center justify-center gap-1`}
          >
            {stat.emoji && <span class="text-lg">{stat.emoji}</span>}{" "}
            {stat.value}
          </div>
