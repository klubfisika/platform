    ...
    >
      ...
      <div class="col-span-6">
        ...
        <div class="flex items-center gap-1.5 mb-1.5 flex-wrap">
          {thread.isSticky && (
            <span
              class={`text-xs px-2 py-0.5 rounded-full font-medium ${THREAD_BADGES.sticky.color}`}
            >
          ...
          {thread.isHot && (
            <span
              class={`text-xs px-2 py-0.5 rounded-full font-medium ${THREAD_BADGES.hot.color}`}
            >
          ...
          {getTypeInfo(thread.type) && (
            <span
              class={`text-xs px-2 py-0.5 rounded-full font-medium ${getTypeInfo(thread.type)!.color}`}
            >
          ...
          {thread.isSolved && (
            <span
              class={`text-xs px-2 py-0.5 rounded-full font-medium ${THREAD_BADGES.solved.color}`}
            >
          ...
          {thread.isLocked && (
            <span
              class={`text-xs px-2 py-0.5 rounded-full font-medium ${THREAD_BADGES.locked.color}`}
            >
      ...
      <div class="col-span-2 flex flex-col items-center justify-center">
        <div
          class={`w-9 h-9 ${THREAD_CONFIG.avatarFallbackBg} rounded-full flex items-center justify-center text-white text-sm font-bold mb-1`}
        >
          {thread.author.avatar || thread.author.name[0]?.toUpperCase()}
        </div>
        <div class="text-sm font-medium text-gray-700 truncate max-w-full">
          {thread.author.name}
        </div>
        {thread.author.rank && (
          <div class={`text-xs ${getRankColor(thread.author.rank)}`}>
            {thread.author.rank}
          </div>
        )}
      </div>
      ...
      <div class="col-span-2 flex flex-col items-center justify-center gap-1">
        ...
        <div class="text-xs text-gray-400">
          {thread.replyCount} {THREAD_LABELS.replies}
        </div>
      </div>
