...
const createOptions = [
  { href: '/feed?compose=true', label: 'Buat Post', icon: '📝', desc: 'Bagikan pemikiran atau pertanyaan' },
  { href: '/projects/new', label: 'Buat Project', icon: '🔬', desc: 'Mulai proyek sains baru' },
  { href: '/discussions/new', label: 'Buat Diskusi', icon: '💬', desc: 'Mulai thread diskusi' },
];
      ...
      {isOpen.value && (
        <>
          ...
          <div class="sm:hidden fixed bottom-0 left-0 right-0 w-full bg-white rounded-t-3xl shadow-2xl z-50 py-4 pb-8 border-t border-gray-100 animate-in slide-in-from-bottom duration-300" style="padding-bottom: env(safe-area-inset-bottom, 2rem);">
            ...
            <div class="px-4">
              <h3 class="text-lg font-semibold text-gray-900 mb-3">Buat Baru</h3>
              {createOptions.map(option => (
                <a
                  key={option.href}
                  href={option.href}
