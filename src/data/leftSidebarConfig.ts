export const LEFT_SIDEBAR_CONFIG = {
  ...
  position:
    "hidden lg:flex flex-col fixed left-0 top-14 h-[calc(100vh-3.5rem)]",
  styling:
    "bg-gray-50 border-r border-gray-100 z-10 transition-all duration-300",
...
export const LEFT_SIDEBAR_STYLES = {
  ...
  nav: "space-y-1",
  navItem:
    "flex items-center gap-3 py-2.5 rounded-lg text-sm font-medium transition",
  ...
  trendingItem:
    "block px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-200 rounded-lg truncate",
  bottomSection: "border-t border-gray-100 p-3",
  bottomSectionMinimized: "px-2",
  backLink:
    "flex items-center gap-2 px-3 py-2 text-sm text-gray-500 hover:bg-gray-200 rounded-lg transition",
  backLinkMinimized:
    "flex justify-center py-2 text-gray-500 hover:bg-gray-200 rounded-lg transition",
  toggleButton:
    "mt-2 w-full flex items-center gap-2 py-2 text-sm text-gray-500 hover:bg-gray-200 rounded-lg transition",
...
export const setSidebarState = (minimized: boolean) => {
  ...
  window.dispatchEvent(
    new CustomEvent("sidebar-toggle", { detail: minimized }),
  );
};
...
export const getNavItemClass = (isActive: boolean, isMinimized: boolean) => {
  ...
  const sizeClass = isMinimized
    ? LEFT_SIDEBAR_STYLES.navItemMinimized
    : LEFT_SIDEBAR_STYLES.navItemExpanded;
  const stateClass = isActive
    ? LEFT_SIDEBAR_STYLES.navItemActive
    : LEFT_SIDEBAR_STYLES.navItemInactive;
