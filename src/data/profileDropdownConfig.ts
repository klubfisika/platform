...
export const PROFILE_DROPDOWN_STYLES = {
  loginButton:
    "bg-green-600 text-white px-4 py-1.5 rounded-lg text-sm hover:bg-green-700 transition",
  profileButton:
    "flex items-center gap-2 p-1 rounded-full transition hover:ring-2 hover:ring-gray-200",
  avatar:
    "w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold shadow",
  backdrop: "fixed inset-0 z-40",
  dropdown:
    "absolute right-0 mt-2 bg-white rounded-xl shadow-lg border border-gray-100 py-2 z-50",
  userInfo: "px-4 py-2 border-b border-gray-100",
  userName: "font-medium text-gray-900 truncate",
  userHandle: "text-sm text-gray-500",
  menuItem:
    "flex items-center gap-3 px-4 py-2 hover:bg-gray-50 text-gray-700 transition",
  separator: "my-2 border-gray-100",
  logoutButton:
    "w-full flex items-center gap-3 px-4 py-2 hover:bg-red-50 text-red-600 transition text-left",
};
